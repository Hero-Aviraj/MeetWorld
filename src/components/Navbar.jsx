    ...
    <>
        ...
        <div> 
          ...
          <button onClick={()=>{setShowfeature(!showfeature);
           ...
           className="flex items-center gap-1 hover:bg-slate-400 rounded transition-colors duration-200  ">
            Features <span className={`text-xs transition-transform duration-200
               ${showfeature? "rotate-180" :"" }`}>▾</span></button>
          {showfeature &&(
            <ul className={`absolute animate-slide-down top-full mt-3 bg-blue-200 
              shadow-lg rounded-lg py-2 w-40 origin-top transition-all duration-200 
       ...
       <div flex> 
        <div className="relative">
          ...
          className="flex items-center gap-1 hover:bg-slate-400 rounded transition-colors duration-200 ">
            More <span className={`text-xs transition-transform duration-200 
              ${showmore? "rotate-180" :"" }`}>▾</span></button>
          {showmore && ( 
            <ul className={`absolute top-full animate-slide-down mt-3 left-0 bg-blue-200 shadow-lg 
              rounded-lg py-2 w-40 rigin-top transition-all duration-200 ${
...
{/*hambarger*/}
      ...
      { <div
        className={`md:hidden absolute top-full left-0 w-full bg-emerald-200 shadow-lg 
          overflow-hidden transition-all duration-300 ease-in-out ${
          mobileOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
        }`}
      >
        <div className="flex flex-col px-6 py-4 gap-4 text-gray-800 font-medium">
          <Link to="/Login" onClick={()=>setMobileOpen(false)}><img src={profileIcon} 
          alt="profile" className="h-8 rounded-lg" /></Link>
          ...
          <div>
            ...
            <div className={`flex flex-col pl-4 overflow-hidden transition-all duration-200 
              ${showmore ? "max-h-40 mt-2" : "max-h-0"}`}>
